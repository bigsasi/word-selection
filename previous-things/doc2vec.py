...
class Doc2Vec(BaseEstimator, TransformerMixin):
    def __init__(self, batch_size=128, window_size=8,
                 ...
                 n_neg_samples=64,
                 optimize='Adagrad',
                 learning_rate=1.0, n_steps=100001, save_path='models/docs_model'):
        ...
        self.word_embeddings = self.doc_embeddings = None # fit() will fill them
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default(), tf.device('/cpu:0'):
            ...
            self.embed = tf.concat(embed, 1)  # antes ponía (1, embed)
            ...
            self.normalized_word_embeddings = tf.div(self.word_embeddings, norm_w)

            norm_d = tf.sqrt(tf.reduce_sum(tf.square(self.doc_embeddings), 1, keep_dims=True))
            self.normalized_doc_embeddings = tf.div(self.doc_embeddings, norm_d)

            # init op
            self.init_op = tf.global_variables_initializer()  # tf.initialize_all_variables()
            # create a saver
    ...
    def _build_dictionaries(self, docs):
        ...
        doc_ids, word_ids, count, dictionary, reverse_dictionary = build_doc_dataset(docs, self.vocabulary_size)
        self.dictionary = dictionary
        self.reverse_dictionary = reverse_dictionary
        self.count = count
        return doc_ids, word_ids

    def fit(self, docs, continue_training=False):
        """
        ...
        for step in range(self.n_steps):
            ...
            if step%10000 == 0 or step == self.n_steps-1:
                # save every some steps and at the end
                self.autosave()
    ...
    def export_embeddings(self, path='trabajos'):
        we_filename = os.path.join(path, 'words_embedding.pickle')
        de_filename = os.path.join(path, 'docs_embedding.pickle')
        with open(we_filename, 'wb') as file:
            pickle.dump(self.word_embeddings, file)
        with open(de_filename, 'wb') as file:
            pickle.dump(self.doc_embeddings, file)

        print('Words embedding exported to', we_filename)
        print('Docs embedding exported to', de_filename)
    ...
    def save(self, path):
        ...
        json.dump(self.dictionary,
                  open(os.path.join(path, 'model_dict.json'), 'w', encoding='utf8'),
                  ensure_ascii=False)
        json.dump(self.reverse_dictionary,
                  open(os.path.join(path, 'model_rdict.json'), 'w', encoding='utf8'),
                  ensure_ascii=False)
    ...
    def restore(cls, path):
        ...
        # bind dictionaries
        estimator.dictionary = json.load(open(os.path.join(path_dir, 'model_dict.json'), 'r', encoding='utf8'))
        reverse_dictionary = json.load(open(os.path.join(path_dir, 'model_rdict.json'), 'r', encoding='utf8'))
